...
RESULT_DIR="${RESULT_DIR:-results/nr_max_1000_pbc_fl_${TARGET_VEHICLES}v}"
...
SCHEDULER_TYPE="${SCHEDULER_TYPE:-ns3::HeapScheduler}"
INDEX_NAME="${INDEX_NAME:-nr_max_${SIM_TIME}_pbc_fl_${TARGET_VEHICLES}v_index.md}"
...
write_index() {
  local index="$RESULT_DIR/$INDEX_NAME"
  {
    ...
    printf '| --- | --- | --- |\n'
    printf '| Per-round PBC+FL power and latency | `%s/nr_max_pbc_fl_rounds_report.md` | `%s/nr_max_pbc_fl_rounds_structured.csv` |\n\n' "$RESULT_DIR" "$RESULT_DIR"
    printf '## Comparison\n\n'
...
python3 "$REPO_ROOT/scripts/format_fl_round_results.py" \
  ...
  --out-prefix "$RESULT_DIR/nr_max_pbc_fl_rounds"
