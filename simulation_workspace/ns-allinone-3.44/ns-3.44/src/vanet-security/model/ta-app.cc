...
TaApp::EnsureKeyPair()
{
  ...
  {
    VanetPowerModel::ScopedTimer timer("ecc_keygen", "ta");
...
TaApp::HandleRead(Ptr<Socket> socket)
{
  ...
  while (packet && packet->GetSize() > 0)
  {
    ...
    while (TryExtractMessage(rxBuffer, header, payload))
    {
      ...
      {
        VanetPowerModel::ScopedTimer timer("cert_sign", "ta");
        cert = EccCrypto::Sign(m_privateKeyPem, toSign.data(), toSign.size());
      }
      ...
      VanetPowerModel::RecordCommunication("cert_resp", "ta", "bs", bytes);
