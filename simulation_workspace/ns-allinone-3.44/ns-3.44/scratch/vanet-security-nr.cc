...
bool
HasCommandLineOption(int argc, char* argv[], const std::string& name)
{
...
int main(int argc, char* argv[])
{
  ...
  double effectiveRateMbps = 1000.0;
  ...
  bool flMasking = true;
  std::string flCsv = "results/fl/nr_fl.csv";
  std::string flStageCsv = "";
  std::string flSecurityMode = "";

  const bool nrFreqOverridden = HasCommandLineOption(argc, argv, "nrFreq");
  const bool nrBwOverridden = HasCommandLineOption(argc, argv, "nrBw");
  const bool nrMuOverridden = HasCommandLineOption(argc, argv, "nrMu");
  const bool infraTxOverridden = HasCommandLineOption(argc, argv, "infraTxPowerDbm");
  const bool effectiveRateOverridden = HasCommandLineOption(argc, argv, "effectiveRateMbps");
  ...
  cmd.AddValue("radioProfile", "Radio profile: 6g_nr_min or 6g_nr_max", radioProfile);
  ...
  std::transform(radioProfile.begin(),
                 radioProfile.end(),
                 radioProfile.begin(),
                 [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
  if (radioProfile == "6g_nr_max")
  {
    if (!nrFreqOverridden)
    {
      nrFreq = 100e9;
    ...
    if (!infraTxOverridden)
    {
      infraTxPowerDbm = 35.0;
    }
    if (!effectiveRateOverridden)
    {
      effectiveRateMbps = 10000.0;
    }
    if (networkLabel.empty())
    {
      networkLabel = "6g_nr_max";
    }
  }
  else
  {
    if (radioProfile != "6g_nr_min")
    {
      NS_LOG_WARN("Unknown radioProfile value, defaulting to 6g_nr_min.");
    ...
    if (!infraTxOverridden)
    {
      infraTxPowerDbm = 30.0;
    }
    if (!effectiveRateOverridden)
    {
      effectiveRateMbps = 1000.0;
    }
    if (networkLabel.empty())
    {
      networkLabel = "6g_nr_min";
    }
  }
  ...
  nrHelper->SetGnbPhyAttribute("TxPower", DoubleValue(infraTxPowerDbm));
  nrHelper->SetGnbPhyAttribute("Numerology", UintegerValue(nrMu));
  nrHelper->SetUePhyAttribute("TxPower", DoubleValue(vehicleTxPowerDbm));
